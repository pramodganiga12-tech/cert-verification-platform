...
export class StudentRepository {
  static async findById(id: string): Promise<StudentRecord | null> {
    ...
    const row = db.prepare('SELECT * FROM students WHERE id = ?').get(id) as StudentRecord | undefined;
    return row || null;
  }

  static async findByIdentifier(institution_id: string, student_identifier: string): Promise<StudentRecord | null> {
    const db = await getDb();
    const row = db.prepare('SELECT * FROM students WHERE institution_id = ? AND student_identifier = ?').get(institution_id, student_identifier) as StudentRecord | undefined;
    return row || null;
  }
  ...
  static async listByInstitution(institution_id: string, limit = 50, offset = 0): Promise<StudentRecord[]> {
    const db = await getDb();
    return db.prepare('SELECT * FROM students WHERE institution_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?').all(institution_id, limit, offset) as StudentRecord[];
  }
}
